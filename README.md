...
├── service-worker.js    # Mechanizm pamięci podręcznej i pracy offline (cache v11)
...
- **Formularz zamówienia**: wybór klienta z listy (automatyczne ukrywanie klientów, którzy mają już przypisane zamówienie w danej szarży) wraz z przyciskiem szybkiego dodawania nowego klienta bezpośrednio w locie (bez utraty już wybranych produktów), przejrzysty i powiększony kreator ilości produktów (+ / −) zoptymalizowany do obsługi palcem na ekranach smartfonów (pełny ekran, duże tap-targety, wyraźne podświetlenie wybranych pozycji), znacznik zamówienia spóźnionego oraz notatki.
...
2. Zwiększ wersję w zmiennej `CACHE_NAME` (np. zmień `wedlinka-v11` na `wedlinka-v12`).
